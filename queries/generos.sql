...
-- relaciona 10 gêneros e o faturamento de vendas que têm (previsões mínima e máxima), excluindo jogos grátis
SELECT DISTINCT genres, SUM(owners_min*price) AS vendas_pessimista, SUM(owners_max*price) AS vendas_otimista, count(appid) AS numero_jogos, (owners_min*price/count(appid)) AS rentabilidade_media FROM jogos WHERE price > 0  GROUP BY genres ORDER BY rentabilidade_media DESC LIMIT 10;
